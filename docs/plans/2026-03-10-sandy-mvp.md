...
**Architecture:** Single Python package with a CLI entry point that discovers plugins from a directory, matches input text against plugin command phrases, and fans out to all matching plugins. No daemon, no state, no server.
...
| `sandy/cli.py` | Entry point: parse args, load plugins, fan out to all matches, print results |
| `sandy/loader.py` | Discover and validate plugins from `sandy/plugins/` directory |
| `sandy/matcher.py` | `find_matches(text, plugins)` — case-insensitive substring matching, returns all matches |
...
git commit -m "feat: add command phrase matcher with fan-out to all matches"
```
