...
- **No retry was added, deliberately.** The cause is a sleeping Mac, not a blip a bounded retry inside a 5-second Slack round trip could ride out; it would only delay the reply.
- Tunnel or origin, per the ticket's own ask: neither process died. dispatchd last started 2026-08-28 23:59 and cloudflared has run since 28 Aug, both continuously across 2026-09-01 — so it was a momentary edge-to-origin loss. Whether the Mac was asleep is no longer answerable; `pmset -g log` only reaches back to 2026-09-08.
- Every capture decision in the plugin now routes through one `_report()`. The first cut of this fix inlined the rule at three call sites and review found the third untested — deleting that guard left all 123 tests green. (638 tests, 88% coverage)
