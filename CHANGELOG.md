...
## 2026-09-19
- Fix #182: every `sandy` invocation wrote `Warning: skipping base.py: missing name, commands, handle` to stderr. `sandy/plugins/base.py` is shared scaffolding for plugin authors and correct as written, so this was a warning about correct behaviour — the kind that trains you to ignore warnings, and noise in front of the user the moment Sandy is embedded in anything wired with `2>&1` (found while researching #181). `sandy help >/dev/null`: 2 warning lines before, 0 bytes of stderr after.
- **The ticket offered two shapes and the choice turns on a reason it did not give.** Option 1 — rename to `_base.py`, skip `_*.py` — hides only files someone remembers to underscore-prefix. Option 2, taken, skips any module declaring **none** of `REQUIRED_ATTRS` and so covers every helper that ever lands in that directory. (The stated objection to option 1 — that `from sandy.plugins.base import SandyPlugin` is a public import, documented under #27 — is true but weaker than it looks: there are zero out-of-tree consumers, itguy and irs both register entry points instead, and a two-line re-export shim would have made the rename non-breaking. The coverage argument is the real one.)
- **The diagnostic is kept, and the first draft of this fix deleted part of it.** A module carrying *some* of the three attributes is claiming to be a plugin and getting it wrong, which is the case the warning exists for. But a file that follows `base.py`'s own instruction — subclass `SandyPlugin`, override `handle` — carries *none* of them at module level, and the file loader appends the module, so such a file has never worked. The first cut silenced it: a wrong diagnostic traded for a missing one, on exactly the author who most needs telling. `_own_plugin_class` now finds a **concrete** `SandyPlugin` subclass or instance alongside `_declares_plugin_attrs`. Abstract ones stay silent, which is what keeps `SandyPlugin` itself — and any intermediate base added beside it — out of the warning stream.
- The guard is on the file path only, never on entry points. Registering under `sandy.plugins` is itself a declaration of intent, so a zero-surface entry point *is* broken and must still say so. Pinned by a test, because moving the check into `_validate_plugin` is a tidier-looking one-line change that silences a real defect — and that test is its sole killer.
- Known and accepted: a file that misspells **all three** attributes (`nmae`/`comands`/`hadnle`) is now silent, where one misspelling still warns. That is the same trade the issue states — a file naming none of them has made no claim — and no check that reads attribute names can tell it from a helper.
- Tests: the boundary is parametrised at *one* declared attribute rather than asserted at `base.py`, so a fix that silenced the noise by widening the skip fails instead of passing; `name = None` is in that table because presence, not truthiness, is the rule, and the `is not None` rewrite otherwise survives the suite. The regression test loads the real shipped plugin directory rather than naming `base.py`, with a non-empty assertion as its reachability control — an empty stderr proves nothing over a loader that returned nothing. (694 passed, measured at head — earlier drafts of this entry said 689 and the commit message said 691, and neither was the count at any revision in the range. Mutation round: 8 production mutants including a full revert to the pre-fix loader, all killed; 1 control green.)
- **Review follow-up: the class-based warning named the wrong fault.** It reached `_validate_plugin` and printed `missing name, commands, handle` — *false*, because the author defined all three as properties exactly as `base.py` instructs. The real defect is that the file loader reads module scope and nothing bridged the instance out. A diagnostic that misnames the fault is this whole arm's justification spent and wasted, so `_load_file_plugins` is now a three-way rather than a two-way: claims nothing → silent, claims only a class → `_warn_missing_bridge` naming the class and the four lines to add, otherwise → `_validate_plugin`. The tests now assert the message rather than just the filename, and assert `missing name, commands, handle` is *absent* — the old assertions passed under the wrong text.
- **A helper that merely imports a plugin class is a helper.** `vars(module)` cannot tell "defined here" from "imported for reuse", so a file doing `from .weather import Weather` was reported as an unbridged plugin, naming a class it does not own. `_own_values` filters on `__module__` (on the type, for instances). Pinned with a sibling that *does* warn as the reachability control — silence in a directory that never loaded proves nothing.
- Docs: `README.md` framed the class-based shape purely as an error state and was the only mention of `SandyPlugin` in the file, while `base.py`'s own docstring still said "subclass this and override `handle()`" with no mention of the bridge. An author reading one concluded the opposite of the other. Both now show the bridge and name it as a file-loader requirement that entry points do not share.
