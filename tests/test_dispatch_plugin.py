...
# ---------------------------------------------------------------------------
# "dispatchd never answered" is not an incident (#190)
# ---------------------------------------------------------------------------


def _capture_list(monkeypatch) -> list[Exception]:
    captured: list[Exception] = []
    monkeypatch.setattr(dispatch_plugin, "capture", lambda e, **k: captured.append(e))
    return captured


def test_cloudflare_origin_error_is_not_captured_to_sentry(http_backend, monkeypatch):
    """SANDY-4: a 530 means Cloudflare never got an answer out of the Mac.

    dispatchd binds to 127.0.0.1 only, so it is structurally incapable of
    emitting this status -- it is the edge's verdict about an origin that did
    not answer, which is the same condition as a refused connection and is an
    expected state here rather than a Sandy defect.
    """
...
def test_cloudflare_origin_error_does_not_blame_dispatchd_for_the_response(
    http_backend, monkeypatch
):
    """The old copy said "dispatchd returned 530", which sends Tom to read
    dispatchd's log for a request that never reached it."""
    _stub_call_raises(monkeypatch, _make_http_error(530))
    text = dispatch_plugin.handle("dispatch pm", "tom")["text"]
    assert "dispatchd returned" not in text
    assert "unreachable" in text
    assert "530" in text


def test_url_error_is_not_captured_to_sentry(http_backend, monkeypatch):
    captured = _capture_list(monkeypatch)
    _stub_call_raises(monkeypatch, urllib.error.URLError("connection refused"))
    dispatch_plugin.handle("dispatch status", "tom")
    assert captured == []
...
def test_timeout_is_not_captured_to_sentry(http_backend, monkeypatch):
    captured = _capture_list(monkeypatch)
    _stub_call_raises(monkeypatch, TimeoutError("timed out"))
    dispatch_plugin.handle("dispatch status", "tom")
    assert captured == []
...
@pytest.mark.parametrize(
    "code, expected_captures",
    [
        (500, 1),  # dispatchd's own failure -- a real incident
        (502, 1),  # ambiguous, so kept as an incident deliberately
        (503, 1),
        (519, 1),  # one below Cloudflare's origin-error range
        (520, 0),  # 520-527 and 530 are generated BY Cloudflare
        (523, 0),
        (527, 0),
        (528, 1),  # one above the range -- not a Cloudflare code
        (529, 1),
        (530, 0),
    ],
)
def test_only_cloudflares_origin_error_range_is_exempt(
    http_backend, monkeypatch, code, expected_captures
):
    """The exemption is a named range, not "any 5xx". Widening it to every 5xx
    would silence a genuinely broken dispatchd, which is the one thing this
    Sentry project exists to tell us about."""
    captured = _capture_list(monkeypatch)
    _stub_call_raises(monkeypatch, _make_http_error(code))
    dispatch_plugin.handle("dispatch pm", "tom")
    assert len(captured) == expected_captures


def test_a_4xx_on_a_read_is_still_captured(http_backend, monkeypatch):
    """HTTPError subclasses URLError, so an unreachability check that tests
    URLError first exempts every HTTP status there is. A 401 on a read is a
    bad key -- the most incident-shaped thing this plugin can hit."""
    captured = _capture_list(monkeypatch)
    _stub_call_raises(monkeypatch, _make_http_error(401))
    dispatch_plugin.handle("dispatch pm", "tom")
    assert len(captured) == 1
...
def test_write_path_also_exempts_cloudflare_origin_errors(http_backend, monkeypatch):
    """The write path already exempts 4xx as control flow; "the origin never
    answered" is the same category and must not depend on which command ran."""
    captured = _capture_list(monkeypatch)

    def raise_code(code):
        def fake(path, *, method="GET", payload=None):
            raise urllib.error.HTTPError("u", code, "x", {}, io.BytesIO(b"<html>origin</html>"))
